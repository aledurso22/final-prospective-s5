...
def make_batch(key, batch=BATCH, length=LENGTH, delay=DELAY):
    """(inputs, target). Channel 0 carries impulses, channel 1 a switch."""
...
def model_apply(params, inputs, use_gates):
    ...
    return jnp.einsum("blf,f->bl", features, params["readout"]), gates, stages
...
def train(use_gates, steps, seed, gate_penalty, learning_rate=3e-2):
    ...
    def step(params, state, batch_key):
        inputs, target = make_batch(batch_key)
...
def evaluate(params, use_gates, seed=99):
    inputs, target = make_batch(jax.random.PRNGKey(seed))
    prediction, gates, stages = model_apply(params, inputs, use_gates)
    error = float(jnp.mean((prediction - target) ** 2))
    # the two components, separately: long-delay memory and lead
    delayed_part = target * (jnp.arange(LENGTH)[None, :] > DELAY)
...
def main():
    ...
    native_params, native_errors = train(False, args.steps, args.seed,
                                         args.gate_penalty)
    gated = evaluate(gated_params, True)
    ...
    report = {
        ...
        "task": "long-delay memory plus switch-edge lead, two channels",
