#!/usr/bin/env bash
# INDEPENDENT-SOURCE REPLICATION of the prospective Momentum DeltaNet
# comparison: checks, measured preflight, 16 source pretraining runs, 12
# development and 18 final continuations. NOT AUTHORIZED TO RUN until
# docs/PROSPECTIVE_MOMENTUM_REPLICATION_PROTOCOL.md is cleared.
#
# ONE hard 600-second budget covers GPU startup, focused checks, preflight,
# ALL 46 runs and 9,200 updates (source pretraining included), evaluation,
# kill grace, both source verifications, the digest and the terminal
...
source "$HERE/prospective_momentum_replication_verify.sh"
export PM_DIGEST_MODULE=experiments.prospective_momentum.replication_summary
...
OUT_ROOT=${OUT_ROOT:-$PROSPECTIVE_RUNS/prospective-momentum-replication}
...
echo "source  : new independent checkpoints in \$RUN_DIR/sources (this study)"
echo "old src : $SOURCE_DIR (READ ONLY; used only by the unchanged checks)"
echo "out     : $RUN_DIR"
echo "logs    : $LOG_DIR"
...
pm_bounded "$STAGE_TERM_AT" "$PM_GRACE_S" \
  "$PY" -u -m pytest tests/test_prospective_momentum.py \
  tests/test_prospective_momentum_replication.py -q -rP --durations=10 \
...
pm_bounded "$STAGE_TERM_AT" "$PM_GRACE_S" \
  "$PY" -u -m experiments.prospective_momentum.replication \
