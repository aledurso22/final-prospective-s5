#!/usr/bin/env bash
# TSS CONTAINMENT: literal TSS residual processing versus the master-law
# processing that starts function-matched to it, on the same Momentum memory,
# with native Momentum and the learned two-tap operator as separate
# references. NOT AUTHORIZED TO RUN until
# docs/PROSPECTIVE_TSS_CONTAINMENT_PROTOCOL.md is cleared.
#
# ONE hard 600-second budget covers GPU startup, focused checks (float64
# module AND the production float32 probe), start-point checks, preflight,
# at most 20 trained continuations (<= 4,000 updates), the frozen-source
...
# sources of run 20260917-011842 are REUSED READ-ONLY. Every step is bounded
# by the same absolute DEADLINE (prospective_momentum_terminal.sh). Preflight
# refuses the batch (INCOMPLETE) if it cannot fit and FAILS it on invalid
# numerical state; nothing is trimmed, retried or extended.
#
#   PROSPECTIVE_MOMENTUM_STATUS=PASS|INCOMPLETE|FAILED  ..._EXIT=0|3|4
# PASS is operational only; the screens are separate performance verdicts.
set -uo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$HERE/cluster_env.sh"
source "$HERE/prospective_momentum_terminal.sh"
export PM_DIGEST_MODULE=experiments.prospective_momentum.tss_containment_summary
...
OUT_ROOT=${OUT_ROOT:-$PROSPECTIVE_RUNS/prospective-tss-containment}
...
# ---- 2a. focused float64 checks, inside the same cap
pm_bounded "$STAGE_TERM_AT" "$PM_GRACE_S" \
...
# ---- 2b. production float32 probe (x64 OFF), inside the same cap
pm_bounded "$STAGE_TERM_AT" "$PM_GRACE_S" \
  env JAX_ENABLE_X64=0 "$PY" -u tests/prospective_tss_containment_float32_probe.py \
  > "$LOG_DIR/float32_probe.log" 2>&1
echo "--- float32 probe ---"; tail -60 "$LOG_DIR/float32_probe.log"
echo "float32 probe: outcome=$PM_OUTCOME rc=$PM_RC (elapsed $(( $(date +%s) - START ))s)"
stage_end float32_probe

# ---- 3. the study: start points, preflight, batch only if it fits
echo "remaining until the study's TERM: $(( STAGE_TERM_AT - $(date +%s) ))s"
pm_bounded "$STAGE_TERM_AT" "$PM_GRACE_S" \
  "$PY" -u -m experiments.prospective_momentum.tss_containment \
...
grep -E "PROSPECTIVE_MOMENTUM_STATUS|SOURCE_UNCHANGED|PREFLIGHT_|\[source\]|\[preflight\]|\[selection\]|\[screen\]|\[!\]" \
